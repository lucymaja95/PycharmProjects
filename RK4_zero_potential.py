...
for j in range(0, t_step):
    for i in range(1, x_step):
        ...
        # elif i == x_step - 1:
            # phi_dx_dx[i, j] = (2 * phi[x_step - 3, j] - 3 * phi[x_step - 1, j] + phi[x_step, j]) / (4 * delta * delta)
        elif i == x_step:
        ...
        l_0 = float(phi_dx_dx[i, j])
        k_1 = float(phi_dot[i, j] + h * l_0 / 2)
        l_1 = float(phi_dx_dx[i, j] + h * k_0 / 2)
        k_2 = float(phi_dot[i, j] + h * l_1 / 2)
        l_2 = float(phi_dx_dx[i, j] + h * k_1 / 2)
        k_3 = float(phi_dot[i, j] + h * l_2)
        l_3 = float(phi_dx_dx[i, j] + h * k_2)
...
np.savetxt('RK4_debug2.dat', DataOut)
